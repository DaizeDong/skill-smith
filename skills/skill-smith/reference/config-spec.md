...
> Hard invariant (skill-smith I6): a config-bearing skill that fails any of E1 to E7 is **rejected** at
> the acceptance gate (G8). Generation != configurable. See `acceptance-gate.md`.

---

## The seven elements (E1 to E7)
...
- **Gate (Step 5, G8):** `python scripts/check_config_conformance.py <repo>` auto-detects whether a repo
  is config-bearing and, if so, checks E1 to E7 with PASS/FAIL per element (including a live deterministic
