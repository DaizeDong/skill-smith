...
| G3 | **Library prompt budget** | every description of OURS <= 180 chars, and NO installed skill of ANY tier past the truncation cutoff. An invisible skill is a capability loss whoever wrote it, so the condition fails regardless of tier; the levers are uninstalling one or trimming ours, never editing a third-party description. The written rule says ~15k chars total; the cutoff OBSERVED on this machine 2026-07-31 is ~20k and four installed skills are already past it. Believe the observed number. | `python scripts/budget_check.py` | silent truncation -> invisible skills |
