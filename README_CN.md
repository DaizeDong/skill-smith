...
**越过截断点的 skill 不分层一律判失败**: 一个 agent 看不见的 skill 就是能力损失, 谁写的描述都一样;
工具会写清真正存在的两根杠杆(卸掉一个, 或者把我们自己的描述改短, 因为截断点是累加值)。
只有每个 skill 的描述**字数上限**仍然只对我们这层判失败, 因为第三方的措辞确实不是使用者能改的。
...
以及这些 guard workflow **在远端默认分支上**到底绿没绿(每个仓每个 workflow 各出一行)。
